...
layout(constant_id = 3) const bool c_ycbcr = true;
...
void main() {
  ...
  if (c_ycbcr) {
    color.r  = texture(sampler2D(s_inputY,    s_sampler), coord).r;
    color.gb = texture(sampler2D(s_inputCbCr, s_sampler), coord).rg - 0.5f;
    color.r  = clamp((color.r - y_min) / (y_max - y_min), 0.0f, 1.0f);
  } else {
    color.rgb = texture(sampler2D(s_inputY, s_sampler), coord).rgb;
  }
