    ...
    for (uint32_t i = 0; i < imageLayers; i++) {
      ...
      for (auto aspects = aspectMask; aspects; ) {
        ...
        if (directCopy) {
          ...
          switch (imageType) {
            case VK_IMAGE_TYPE_1D: srcData += bytesPerRow;
            case VK_IMAGE_TYPE_2D: srcData += blockCount.height * rowPitch;
            case VK_IMAGE_TYPE_3D: srcData += blockCount.depth * slicePitch;
            default: ;
          }
        } else {
          for (uint32_t i = 0; i < blockCount.depth; i++) {
            ...
            switch (imageType) {
              case VK_IMAGE_TYPE_1D: srcData += bytesPerRow;
              case VK_IMAGE_TYPE_2D: srcData += blockCount.height * rowPitch;
              case VK_IMAGE_TYPE_3D: srcData += slicePitch;
              default: ;
            }
